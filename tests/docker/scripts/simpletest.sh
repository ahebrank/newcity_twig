...
docker exec -it module_testing bash -c "sudo -u www-data php core/scripts/run-tests.sh --browser --url http://drupal/ --class \"\Drupal\Tests\newcity_twig\Functional\NewcityTwigTest\""